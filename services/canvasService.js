...
export function initCanvasService() {
    try {
        ...
        // Batasi input hanya menerima SVG
        refImageInput.setAttribute('accept', '.svg');
...
// --- Upload Referensi Image (SVG Murni) ---
function attachRefImageEvent() {
    refImageInput.addEventListener('change', (e) => {
        try {
            ...
            // 1. Validasi ekstensi & MIME type (Hanya SVG)
            if (file.type !== 'image/svg+xml' && !file.name.endsWith('.svg')) {
                alert('Hanya file .svg yang diizinkan sebagai gambar referensi agar output tetap murni vektor!');
                e.target.value = '';
                return;
            }
            ...
            if(fabricCanvas) {
                ...
                // 2. Gunakan loadSVGFromURL untuk menjaga struktur vektor
                fabric.loadSVGFromURL(url, (objects, options) => {
                    const group = fabric.util.groupSVGElements(objects, options);
                    
                    // Skala agar pas di tengah kanvas dengan ukuran 60% dari lebar
                    const scale = Math.min(canvasWidth / group.width, canvasWidth / group.height) * 0.6;
                    group.scale(scale);
                    
                    group.set({
                        left: (canvasWidth - group.width * scale) / 2,
                        top: (canvasWidth - group.height * scale) / 2
                    });
